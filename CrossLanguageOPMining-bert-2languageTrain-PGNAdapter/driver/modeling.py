...
def load_tf_weights_in_bert(model, tf_checkpoint_path):
    ...
    for name, array in zip(names, arrays):
        ...
        if any(n in ["adam_v", "adam_m",
                     "global_step", "output_bias", "output_weights"] for n in name):
...
class BertConfig(object):
    ...
    def __init__(self,
        ...
        if isinstance(vocab_size_or_config_json_file, str) or (sys.version_info[0] == 2
            ...
            self.adapter_prediction = adapter_prediction
...
class BertSelfOutput(nn.Module):
    def __init__(self, config):
        ...
        if config.use_adapter:
            adapter = Adapter(config) if not config.use_language_emb else AdapterWithParameterGen(config)
            if config.num_adapters > 1:
                self.adapter = nn.ModuleList([copy.deepcopy(adapter) for _ in range(config.num_adapters)])
            else:
        ...
        if self.adapter is not None:
            if self.config.num_adapters > 1:
                if self.adapter_prediction == 'average':
                    ...
                    hidden_states = self.adapter[adapter_idx](hidden_states)
            else:
                hidden_states = self.adapter(hidden_states) \
                if isinstance(self.adapter, Adapter) else self.adapter(hidden_states, lang_emb)
...
class BertAttention(nn.Module):
    ...
    def forward(self, input_tensor, attention_mask, adapter_idx=None, lang_emb=None):
        self_output = self.self(input_tensor, attention_mask)
        attention_output = self.output(self_output, input_tensor, adapter_idx, lang_emb)
...
class BertLayer(nn.Module):
    ...
    def forward(self, hidden_states, attention_mask, adapter_idx=None, lang_emb=None):
        attention_output = self.attention(hidden_states, attention_mask, adapter_idx, lang_emb)
        intermediate_output = self.intermediate(attention_output)
        layer_output = self.output(intermediate_output, attention_output, adapter_idx, lang_emb)
        return layer_output
...
class AdapterWithParameterGen(nn.Module):
    def __init__(self, config):
        ...
        self.config = config
        self.low_rank_dim = config.language_emb_size
        self.down_W = nn.Parameter(torch.zeros(self.low_rank_dim, config.hidden_size, config.adapter_size))
        self.down_b = nn.Parameter(torch.zeros(self.low_rank_dim, config.adapter_size))

        self.activation = ACT2FN[config.adapter_act] if isinstance(config.adapter_act, str) else config.adapter_act

        self.up_W = nn.Parameter(torch.zeros(self.low_rank_dim, config.adapter_size, config.hidden_size))
        self.up_b = nn.Parameter(torch.zeros(self.low_rank_dim, config.hidden_size))
    ...
    def forward(self, hidden_states, lang_emb=None):
        ...
        up_w = torch.matmul(lang_emb, self.up_W.view(self.low_rank_dim, -1)).view(self.config.adapter_size, self.config.hidden_size)
    ...
    def init_weights(self, config):
        # Slightly different from the TF version which uses truncated_normal for initialization
        # cf https://github.com/pytorch/pytorch/pull/5617
        self.down_W.data.normal_(mean=0.0, std=config.adapter_initializer_range)
        self.up_W.data.normal_(mean=0.0, std=config.adapter_initializer_range)
...
class BertEncoder(nn.Module):
    ...
    def forward(self, hidden_states, attention_mask, output_all_encoded_layers=True, adapter_idx=None, lang_emb=None):
        all_encoder_layers = []
        for layer_module in self.layer:
            hidden_states = layer_module(hidden_states, attention_mask, adapter_idx, lang_emb)
...
class BertPreTrainedModel(nn.Module):
    ...
    def from_pretrained(cls, pretrained_model_name_or_path, state_dict=None, cache_dir=None,
                        verbose=True, from_tf=False, *inputs, **kwargs):
        """
...
class BertModel(BertPreTrainedModel):
    ...
    def __init__(self, config):
        ...
        self.bert_layers = config.num_hidden_layers

        if config.use_adapter:
            for param in self.parameters():
                param.requires_grad = False
            for name, sub_module in self.named_modules():
                if isinstance(sub_module, (Adapter, BertLayerNorm, AdapterWithParameterGen)):
                    for param_name, param in sub_module.named_parameters():
                        param.requires_grad = True

    def forward(self, input_ids, token_type_ids=None, attention_mask=None, output_all_encoded_layers=True, adapter_idx=None, lang_embedding=None, bert_pieces=None):
